...
JWTHandler FJWT::f;
String FJWT::projectEmail;
String FJWT::privateKey;
String FJWT::scope;
uint32_t FJWT::unixTime;
uint8_t FJWT::buffer[2000];
...
/*------ FreeRTOS Task ------*/
void FJWT::jwtTask(void *parameter) {
  if (unixTime > 1600000000) {
    ...
    if (rc != 0) {
      f("Failed to parse private key (err_code: " + String(rc, HEX) + ")", false);
    }
    ...
    if (rc != 0) {
      f("Failed to digest content (err_code: " + String(rc, HEX) + ")", false);
      vTaskDelete(NULL);
    }
    ...
    if (rc != 0) {
      f("Failed to sign content (err_code: " + String(rc, HEX) + ")", false);
      vTaskDelete(NULL);
    }

    String signature = base64_encode(buffer, retSize);
    mbedtls_pk_free(&pk_context);
    f(content + "." + signature, true);
    vTaskDelete(NULL);
  } else {
    f("Unix timestamp is out of date(err_code: 0)", false);
    vTaskDelete(NULL);
  }
}
...
/*------ FirebaseJWT ------*/
FJWT& FJWT::getInstance() {
  static FJWT instance;
  return instance;
};

void FJWT::begin(JWTHandler f) {
  this->f = f;
}

bool FJWT::encode(String projectEmail, String privateKey, String scope, uint32_t unixTime) {
  if (!f) return false;
  this->projectEmail = projectEmail;
  this->privateKey = privateKey;
  this->scope = scope;
  this->unixTime = unixTime;
  xTaskCreate(jwtTask, "jwtTask", 10000, NULL, 1, NULL);
  return true;
}
