...
struct MessageUtil {
  protected:
    ...
    String jsonArray(String key, vector<String> arr, bool isDuration = false) const {
      if (arr.empty()) {
        return "";
      } else {
        String str = "\"" + key + "\":[";
...
struct AndroidNotification : protected MessageUtil {
  ...
  vector<String> bodyLocArgs;
  vector<String> titleLocArgs;
  vector<String> vibrateTimings;
  AndroidColor color;
...
class FCM : public Message {
  protected:
    String buildMessage(Message message, bool validateOnly = false);
};
